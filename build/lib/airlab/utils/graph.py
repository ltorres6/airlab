...
class Graph:
    ...
    def _create_graph_2d(self):
        ...
        element_1 = MatrixDiagonalElement(
            np.asarray(self._edge_index_D1),
            np.ones(len(self._edge_index_D1[0])),
            th.tensor(1, dtype=th.int64),
            dtype=self.dtype,
            device=self.device,
        )
        element_2 = MatrixDiagonalElement(
            np.asarray(self._edge_index_D2),
            np.ones(len(self._edge_index_D2[0])),
            th.tensor(self._graph_size[0], dtype=th.int64),
            dtype=self.dtype,
            device=self.device,
        )

        self.laplace_matrix = LaplaceMatrix(
            self._number_of_nodes, [element_1, element_2], dtype=self.dtype, device=self.device
        )
    ...
    def _create_graph_3d(self):
            ...
            for y in range(self._graph_size[1]):
                for x in range(self._graph_size[0]):
                    ...
                    if x + 1 < self._graph_size[0]:
                        ...
                        self._edge_index_D1[3].append(
                            x + y * self._graph_size[0] + z * self._graph_size[0] * self._graph_size[1]
                        )

                    if y + 1 < self._graph_size[1]:
                        self._edge_index_D2[0].append(x)
                        self._edge_index_D2[1].append(y + 1)
                        self._edge_index_D2[2].append(z)
                        self._edge_index_D2[3].append(
                            x + y * self._graph_size[0] + z * self._graph_size[0] * self._graph_size[1]
                        )
        ...
        element_3 = MatrixDiagonalElement(
            np.asarray(self._edge_index_D3),
            np.ones(len(self._edge_index_D3[0])),
            th.tensor(self._graph_size[0] * self._graph_size[1], dtype=th.int64),
            dtype=self.dtype,
            device=self.device,
        )

        self.laplace_matrix = LaplaceMatrix(
            self._number_of_nodes, [element_1, element_2, element_3], device=self.device
        )
