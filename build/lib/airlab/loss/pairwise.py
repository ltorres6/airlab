...
class _PairwiseImageLoss(th.nn.modules.Module):
    def __init__(self, fixed_image, moving_image, fixed_mask=None, moving_mask=None, size_average=True, reduce=True):
        ...
        self._grid = T.utils.compute_grid(
            self._moving_image.size, dtype=self._moving_image.dtype, device=self._moving_image.device
        )
...
class LCC(_PairwiseImageLoss):
    def __init__(
        ...
        if kernel_type == "box":
            ...
            self._kernel = th.ones(*kernel_size.tolist(), dtype=self._dtype, device=self._device) / float(
...
class MI(_PairwiseImageLoss):
    ...
    def __init__(
        ...
        self._bins_fixed_image = th.linspace(
            self._background_fixed, self._max_f, self.bins, device=fixed_image.device, dtype=fixed_image.dtype
        ).unsqueeze(1)

        self._bins_moving_image = th.linspace(
            self._background_moving, self._max_m, self.bins, device=fixed_image.device, dtype=fixed_image.dtype
        ).unsqueeze(1)
    ...
    def forward(self, displacement):
        ...
        sample = (
            th.zeros(number_of_pixel, device=self._fixed_image.device, dtype=self._fixed_image.dtype).uniform_()
...
class NGF(_PairwiseImageLoss):
    ...
    def __init__(
        self, fixed_image, moving_image, fixed_mask=None, moving_mask=None, epsilon=1e-5, size_average=True, reduce=True
    ):
...
class SSIM(_PairwiseImageLoss):
    ...
    def __init__(
        ...
        if kernel_type == "box":
            kernel_size = sigma * 2 + 1
            self._kernel = th.ones(*kernel_size.tolist()) / float(np.product(kernel_size) ** 2)
        elif kernel_type == "gaussian":
    ...
    def forward(self, displacement):
        ...
        variance_moving_image = F.conv2d(self._warped_moving_image.pow(2), self._kernel) - (mean_moving_image.pow(2))
