...
def upsample_displacement(displacement, new_size, interpolation="linear"):
    ...
    if type(displacement) == iutils.Displacement:
        displacement = th.squeeze(displacement.image)
    else:
...
def warp_image(image, displacement):
    ...
    if th.is_complex(image.image):
        dtype = th.float32
    else:
        dtype = image.dtype
    grid = compute_grid(image_size, dtype=dtype, device=image.device)

    # warp image
    if th.is_complex(image.image):
        warped_image_r = F.grid_sample(image.image.real, displacement + grid)
        warped_image_i = F.grid_sample(image.image.imag, displacement + grid)
        warped_image = th.complex(warped_image_r, warped_image_i)
    else:
