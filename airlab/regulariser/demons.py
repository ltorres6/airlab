    ...
    def update(self, data):
        ...
        if self._dim == 2:
            ...
            for i, diag in enumerate(self._laplace_matrix.diag_elements):
                ...
                intensyties_B = self._image[
                    0, 0, diag.edge_index[0] - one[0], diag.edge_index[1] - one[1], diag.edge_index[2] - one[2]
                ]
    ...
    def update(self, data):
        ...
        if self._dim == 2:
            ...
            for i, diag in enumerate(self._laplace_matrix.diag_elements):
                ...
                if self._edge_mean:
                    diag.edge_values = diag.edge_values * self._edge_window + th.round(weight) * (1.0 - self._edge_window)
                else:
            ...
            for i, diag in enumerate(self._laplace_matrix.diag_elements):
                ...
                intensyties_A = self._image[diag.edge_index[0], diag.edge_index[1]]
                intensyties_B = self._image[
                    diag.edge_index[0] - one[0], diag.edge_index[1] - one[1], diag.edge_index[2] - one[2]
                ]
                ...
                displacement_diff = displacement_A - displacement_B
                displacement_diff = th.sqrt(
                    displacement_diff[0, :] ** 2 + displacement_diff[1, :] ** 2 + displacement_diff[2, :] ** 2
                )
                displacement_diff = th.exp(-self._scale_disp_diff * displacement_diff)

                norm_disp_A = th.sqrt(
                    displacement_A[0, ...] ** 2 + displacement_A[1, ...] ** 2 + displacement_A[2, ...] ** 2
                )
                norm_disp_B = th.sqrt(
                    displacement_B[0, ...] ** 2 + displacement_B[1, ...] ** 2 + displacement_B[2, ...] ** 2
                )

                image_gradient_A = self._image_gradient[diag.edge_index[0], diag.edge_index[1], :]
                image_gradient_B = self._image_gradient[
                    diag.edge_index[0] - one[0], diag.edge_index[1] - one[1], diag.edge_index[2] - one[2], :
                ]

                norm_A = th.sqrt(
                    image_gradient_A[..., 0] ** 2 + image_gradient_A[..., 1] ** 2 + image_gradient_A[..., 2] ** 2
                )
                norm_B = th.sqrt(
                    image_gradient_B[..., 0] ** 2 + image_gradient_B[..., 1] ** 2 + image_gradient_A[..., 2] ** 2
                )
...
class GraphDiffusionRegulariser(_DemonsRegulariser):
    ...
    def get_edge_image(self):
        ...
        return iu.Image(
            main_diag_laplace.unsqueeze_(0).unsqueeze(0),
            self._image_size,
            self._pixel_spacing,
            th.zeros(len(self._image_size)),
        )  # only zero origin supported yet
    ...
    def regularise(self, data):
        for parameter in data:
            ...
            with th.no_grad():
                ...
                for i in range(dim):
                    mat.expm_krylov(
                        self._graph.laplace_matrix,
                        parameter.data[i, ...].view(-1),
                        phi=self._phi,
